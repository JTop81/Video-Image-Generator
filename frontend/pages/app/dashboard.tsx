...
const Page = () => {
  ...
  const generateList = [
    { title: "Create a new Video", type: SelectType.VIDEO, link: "/app/createVideo" },
    { title: "Create a Text to Speech", type: SelectType.TTS, link: "/app/createSpeech"},
  ];

  const handleOpenSelectionModal = (type: SelectType, link: string) => {
        ...
        {generateList.map((item) => (
          <Button key={item.title} duration={Math.floor(Math.random() * 10000) + 10000}>
            <Card
              ...
              onClick={() => handleOpenSelectionModal(item.type, item.link)}
            >
              <CardContent
                sx={{
                  display: "flex",
